...
<ROUTINE V-INVENTORY ("AUX" OBJ NXT)
...
<ROUTINE PRINT-CONTENTS (CONT "OPT" (RECUR? <>) (CNT 0) "AUX" OBJ (1ST? T))
	<SET OBJ <FIRST? .CONT>>
	<COND
		(.OBJ
			<REPEAT ()
...
"You claw at" the ,PRSO " for a few moments but find nothing of interest." CR
		...
		(<AND <MC-PRSO? ,TH-GROUND>
			...
			; "Bob"
			<TELL
"You dig at" the ,PRSO " for a few moments but find nothing of interest." CR
			>
		)
		(T
...
The+verb ,PRSO "do" "n't look very appetizing to" the ,WINNER "." CR
		...
		(T
			<TELL The+verb ,WINNER "are" "n't hungry right now." CR>
		)
	>
>
...
<ROUTINE V-EXTINGUISH ()
	<COND
		(<AND <EQUAL? ,P-PRSA-WORD ,W?PUT ,W?STICK>
				<MC-PRSI? <> ,ROOMS>
				<MC-PRSO? ,TH-HEAD ,TH-LEGS ,TH-HANDS>
			>
...
<ROUTINE PRE-PUT ()
	<COND
		(<MC-PRSO? ,TH-HEAD ,TH-HANDS ,TH-LEGS ,TH-MOUTH>
			<RT-WONT-HELP-MSG>
			<RTRUE>
		)
		(<AND <VERB? PUT>
		...
		(<MC-FORM? ,K-FORM-TURTLE>
			<COND
				...
				(<NOT <FSET? ,HERE ,FL-WATER>>
					<TELL
"You plod off in that " D ,INTDIR " for some time. After a while you look
up and discover that you have made very little progress." CR
					>
				)
			>
		)
	...
	<COND
		(<MC-PRSO? <> ,ROOMS>
			<COND
				...
				(T
					<RT-AUTHOR-ON>
					<TELL "What direction do you want to " vw "?">
					<RT-AUTHOR-OFF>
					<RFATAL>
				)
			>
